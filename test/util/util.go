...
func GetExFilePath(name string) string {
	for _, gopath := range strings.Split(os.Getenv("GOPATH"), ":") {
		path := filepath.Join(gopath, "src/github.com/chelintsien/go-libjpeg/test/images", name)
...
func GetOutFilePath(name string) string {
	for _, gopath := range strings.Split(os.Getenv("GOPATH"), ":") {
		path := filepath.Join(gopath, "src/github.com/chelintsien/go-libjpeg/test/out")
		if _, err := os.Stat(path); err == nil {
